@tool
class_name OrbitMotionBezier extends Node3D
...
@export var t_anim : float :
	set(val):
		t_anim = val
		if anim_player:
			anim_player.speed_scale = 1.0/t_anim
...
func reconstruct_trajectory_track(anim, track_id, h, q, mu):
	var baked_points = rd.curve.get_baked_points()
	var num_baked_points = baked_points.size()
	...
	match trajectory_type:
		"elliptic":
			theta_max = PI
			var n = sqrt(mu / a**3)
			var E_anomaly = 2 * atan2(sqrt((1.0-e)/(1.0+e)) * sin(theta_max/2), cos(theta_max/2))
			t_max = (E_anomaly - e * sin(E_anomaly)) / n
			
		"hyperbolic":
			theta_max = rd.theta_max
			var n = sqrt(mu / abs(a)**3.0)
			var F_anomaly = 2.0 * atanh(sqrt((e-1.0)/(e+1.0)) * tan(theta_max/2.0))
	...
	for i in range(num_baked_points):
		var theta_i = atan2(baked_points[i].y, baked_points[i].x)
		#var r_i = baked_points[i].length()
		var t_i
		match trajectory_type:
			'elliptic':
				var n = sqrt(mu / a**3)
				var E_anomaly = 2.0 * atan2(sqrt((1.0-e)/(1.0+e)) * sin(theta_i/2), cos(theta_i/2))
				t_i = (E_anomaly - e * sin(E_anomaly)) / n
			
			'hyperbolic':
				var n = sqrt(mu / abs(a)**3.0)
				var F_anomaly = 2.0 * atanh(sqrt((e-1.0)/(e+1.0)) * tan(theta_i/2.0))
